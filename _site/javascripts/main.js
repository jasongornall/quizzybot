...
var MY_user_data, calculatePonts, cleanup, getProfileData, handleAuth, handleLink, handleRoute, intervals, renderTimeouts, route_url, setTimeoutStr, showError, timeouts;
...
setTimeoutStr = function(str, length, func) {
  clearTimeout(renderTimeouts[str]);
  return renderTimeouts[str] = setTimeout(func, length * 1000);
};
...
handleRoute = function(route, $el) {
  ...
  switch (route) {
    case '/old':
      console.log('a');
      return firebase.database().ref("old_questions").on('value', function(data) {
        return $el.html(teacup.render(function() {
          var key, question, _ref, _results;
          _ref = data.val();
          _results = [];
          for (key in _ref) {
            question = _ref[key].question;
            console.log(key, question, 'wakka');
            _results.push(div('.question', {
              'data-key': key
            }, function() {
              h1(function() {
                return 'Question';
              ...
              div('.text', function() {
                return "" + question["public"].question;
              });
              h3(function() {
                return 'Answer';
              });
              div('.points', function() {
                return "" + question["public"].user.answer;
              });
              h3(function() {
                return 'pointed gained';
              });
              return div('.points', function() {
                return "" + (question["public"].end_ts - question["public"].ts);
              });
            }));
          }
          return _results;
    ...
    case '/store':
      return firebase.database().ref("store").on('value', function(data) {
          ...
          return div('.store-front', function() {
            return div('.store-item', {
              ...
              div('.header', function() {
                ...
                div('.description', function() {
                  return 'Add your own question into the quizzy game!';
                });
                return div('.cost', function() {
                  return "100 QuizzyPoints";
                });
              });
              return div('.form', function() {
                div(function() {
                  return 'Riddle Title';
                });
                textarea('.riddle');
                div(function() {
                  return 'hint 1 (after 24 hours) (optional)';
                });
                textarea('.hint.1');
                div(function() {
                  return 'hint 2 (after 48 hours) (optional)';
                });
                textarea('.hint.2');
                div(function() {
                  return 'hint 3 (after 72 hours) (optional)';
                });
                textarea('.hint.3');
                div(function() {
                  return 'answer';
        ...
        $el.find('.store-item .header').on('click', function(e) {
          $el = $(e.currentTarget);
          return $el.siblings('.form').slideToggle();
        });
        return $el.find('.store-item[data-type=riddle] .submit').on('click', function(e) {
          var $form, key, uid;
          $form = $(e.currentTarget).closest('.form');
          key = firebase.database().ref().child('question_bucket').push().key;
          uid = firebase.auth().currentUser.uid;
          return firebase.database().ref("users/" + uid).once('value', function(profile_doc) {
            var current_points, updates;
            current_points = profile_doc.child('points').val() || 0;
            updates = {};
    ...
    case '/':
      ...
      firebase.database().ref("active_question/public").on('value', function(data) {
        if ($el.find('.question .wrapper').data('ts') === data.child('ts').val()) {
          return;
        }
        return $el.find('.question').html(teacup.render(function() {
          return div('.wrapper', {
            'data-ts': data.child('ts').val()
          }, function() {
            div(function() {
